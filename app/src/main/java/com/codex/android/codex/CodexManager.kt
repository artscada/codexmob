...
class CodexManager(private val context: Context) {
    ...
    companion object {
        ...
        private const val BUNDLED_CODEX_ASSET = "prebuilt/codex-aarch64-unknown-linux-musl"
    ...
    fun isInstalled(): Boolean {
        return hasValidBinary() || ensureBundledBinaryIfAvailable()
    }
    ...
    fun verifyBinary(): Boolean {
        if (!hasValidBinary() && !ensureBundledBinaryIfAvailable()) return false
    ...
    suspend fun downloadWithProgress(
        ...
        if (ensureBundledBinaryIfAvailable()) {
            val size = codexBinary.length()
            onProgressCallback?.invoke(size, size)
            Log.i(TAG, "Использован встроенный бинарник Codex CLI")
...
""".trimIndent()
    ...
    private fun hasValidBinary(): Boolean {
        return codexBinary.exists() && codexBinary.canExecute() && codexBinary.length() > 10_000_000
    }

    private fun ensureBundledBinaryIfAvailable(): Boolean {
        if (hasValidBinary()) return true

        return try {
            codexDir.mkdirs()
            context.assets.open(BUNDLED_CODEX_ASSET).use { input ->
                FileOutputStream(codexBinary).use { output ->
                    input.copyTo(output)
                }
            }
            codexBinary.setExecutable(true, false)
            if (verifyElfHeader(codexBinary)) {
                recordInstalledVersion(CODEX_VERSION)
                Log.i(TAG, "Встроенный Codex CLI установлен: ${codexBinary.length()} bytes")
                true
            } else {
                codexBinary.delete()
                false
            ...
            Log.w(TAG, "Не удалось установить встроенный Codex CLI", e)
            codexBinary.delete()
            false
        }
    }

    private fun verifyElfHeader(file: File): Boolean {
        return try {
